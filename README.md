...
| **② 剪枝臂** | `agent/pre-step` | 存量工具输出 | **预算驱动**：会话总量超 `budgetTokens` → 按体积降序批量替换为摘要+句柄，剪到 `targetTokens` 以下；**不按单条的尺寸/年龄设死规则**（第一轮就可能来超大输出，尺寸与语义价值无关） |
...
node core/cli.mjs report                        # 五段账本：缓存/静态层/剪枝/压缩/存储
...
budgetTokens: 100000          # 剪枝触发水位（会话总量）
...
| 套件 | 数量 | 结果 | 覆盖 |
|---|---|---|---|
| core 测试 | 11 | ✅ 11/11 | 压缩往返无损（含中文/emoji）、内容寻址去重、TTL 清理、容量上限最旧优先、清扫节流、计量轮转、**多帧 zstd 会话日志恢复**、非法输入 |
| **会话存活回放** | 5 | ✅ 5/5 | 用 DSH 自己的 `foldSurface` 回放含替换事件的日志：折叠接受、**surface 只剩替换节点**、原文仍在日志（可恢复）、折叠确定性、**反向校验生效**（越界改写被拒、缺 `sourceEventSeqs` 被拒） |
| 适配器契约 | 15 | ✅ 15/15 | 直通/透传分支、shadow 不替换、active 替换+句柄可回取、失败静默、观测臂记账、剪枝预算/最小/最大优先/最新保护/冷却、静态层确定性、分臂模式、非法配置拒绝 |
...
- **记忆提取尚未实现**（Phase 3）：当前只有压缩/剪枝/观测，跨会话记忆与对话轮折叠是下一步
