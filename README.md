...
记忆注入的项目作用域（生产 blocker 修复）：记忆库是全局的，但**任务事实按项目隔离**——
`search` 传 `project` 即进入 `same` 口径（同项目 + 无 project 的全局条目 + `profile` 画像条目），
`projectScope: 'all'` 才是旧的池化行为。CLI 预览：`lcm memory search --query ... --project <cwd> [--scope all]`。
实测（金标库 61 对）：池化口径 recall 98.4% / 跨项目污染 22.7%（276 个外项目槽位）；
隔离后 recall **100%** / 污染 **0%**，另有 6154 条外项目候选被挡。

提取器噪声过滤（形态过滤，非分数门槛）：`narrativeNoiseReason` 按语义形态拦噪声——
行为宣告（「我先核实…」）、对话管理（「一句话总结你的疑问」）、规划建议（「我建议的顺序…」）、
目录树行、短冒号尾碎片；assistant 专属形态（选项菜单「A. 先切 active…」、你-向建议
「你现在重启一次…」、要我-offer）只在 `role: 'assistant'` 时生效——**用户指令是真实决策，必须保留**。
实测（6 会话/1106 轮）：候选 1173→862（−26.5%）、低信号碎片 22.6%→14.0%（余下为合法低信号条目）、
入库 80→64 条且抽查零叙述垃圾；金标库重建后跨会话 recall 95.8%（48 对，旧 61 对里含噪声条目
产生的虚假配对）、击穿 0、污染 0。
现场实证：html_to_md 的查询在池化口径下会召回 libre_quant 的交易规则（溢价规则、`/api/live` 现价）。
...
实测本机：144 真实会话净省 15.0%（占反事实），内置载荷口径交叉验证 ratio 1.02（14.7% vs 15.0%），并与 bench-all 11.9% 同向同量级；`node scripts/value-xcheck.mjs` 把互证变成退出码。
...
| 套件 | 数量 | 结果 | 覆盖 |
|---|---|---|---|
| core 测试 | 96 | ✅ 96/96 | 压缩往返无损（含中文/emoji）、内容寻址去重、TTL 清理、容量上限最旧优先、清扫节流、计量轮转、**全局/项目根合并读取**、**旧口径 fresh 归一化 + 按项目过滤**、**记忆写入决策四分支/禁写过滤/质量门槛分层/检索预算/注入确定性/提取幂等（表格行/引用前缀/冗余 subject 归零）/outbox + mock HTTP 同步**、**画像：harness 伪装消息过滤/续接重放去重/意图分类/句式指纹/预算截断保闭合/热路径只读缓存**、多帧 zstd 会话日志恢复、非法输入 |
