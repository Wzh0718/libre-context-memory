...
```
实际（装 lcm）：      每请求当量 33,766 ｜ 命中率 93.7%
反事实（不装 lcm）：  每请求当量 34,845
治理掉的存量：5 次真剪枝/压缩，累计 49,296 tokens 退出热区
毛节省：+250,377 当量（+3.1%）
击穿成本：15 次前缀打穿，其中归因 lcm 剪枝的 3 次（510,930 当量）
净收益：−260,553 当量（−3.2%）
盈亏平衡：改写历史需后续 ≥104 个请求才回本
```

**这个负数是本项目最重要的实测结论**：改写历史 = 打穿一次前缀（代价 ≈ 全价重发整段上下文），
而收益是「已治理体积 × 缓存价 × 后续轮数」——以本窗口实测参数（上下文 ~250k tokens、
一次剪掉 49k）计算，**需要 ≥104 个后续请求才回本**。

→ 由此得出的设计修正：**剪枝应该搭便车，而不是主动制造击穿**（见下方「待办」）。
...
- **主动剪枝目前是净负的**（实测 −3.2%）：它有意识地打穿前缀，回本需要 ≥104 个后续请求。下一步应改为「搭便车剪枝」——只在缓存本来就要失效时（内置折叠前后、已观测到击穿后）才动手，那时改写历史是免费的
