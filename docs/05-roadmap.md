...
- 毛节省 +250,377 当量（+3.1%），**主动剪枝引起的击穿成本 510,930 当量（3 次）**，净 **−3.2%**
- **盈亏平衡：改写历史需 ≥104 个后续请求才回本**（击穿 ≈ 全价重发整段 ~250k 上下文；收益 = 治理体积 × 0.1 × 后续轮数）
- → **已实施设计修正**：默认 `pruneProactive: false`，改为「搭便车剪枝」——在 `compaction/*` 事件后、
  或观测到 `fresh > bustThresholdTokens` 的击穿后，才由 `agent/pre-step` 改写历史；此时前缀本来就要失效，
  改写历史的成本归零，毛节省 +3.1% 直接落袋。主动路径仍保留给超长会话显式开启。
